...
def switch_to_basic_mode(context, mode):
    context.execute_steps(u'* Click "%s" in GApplication menu' % mode)
...
def calculate(context, expression):
    typeText(expression)
    context.app.instance.child('result').click()
...
def verify_result(context, result):
    ...
    actual = context.app.instance.child(roleName='editbar').text.replace('\xe2\x88\x92','-')
...
def error_message_displayed(context, message):
    actual = context.app.instance.child(roleName='editbar').parent.textentry('').text
